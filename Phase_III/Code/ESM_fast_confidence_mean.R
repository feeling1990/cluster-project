...
ESM_fast_confidence=function(xx,obj,clusternum,maxiter,truelabel,max_feature_num,threshold1,threshold2){
  ...
  for (i in 2:maxiter){
    ...
    index = which.min(RI_mean)
    if(abs(mean(RI_matrix[i,]-RI_matrix[i-1,])) < threshold1){
      Outlier_Candidates = c(Outlier_Candidates,RI_info[[3]])
    }
    if((abs(mean(RI_matrix[i,]-RI_matrix[i-1,]))<threshold1)&(RI_mean[index]<threshold2)&(i>5)){
  ...
  if (length(misclassified)>length(mylabel)/2){
    mylabel_adjust = shuffle(mylabel)
    misclassified = which(truelabel != mylabel_adjust)
  }
  results = list()
